...
<html><body>
<div class="row">
  <a class="job_title_link" href="/apply/jobs/details/ABC123?&">Inginer Ofertare Energetic</a>
</div>
<div class="row">
  <a class="job_title_link" href="/apply/jobs/details/DEF456?&">SCADA Engineer</a>
</div>
</body></html>
"""
...
<html><body><table>
  <tr>
    <td><a class="job_title_link" href="/apply/jobs/details/AAA111?&">Job A</a></td>
    <td>Cluj-Napoca, Cluj, Romania</td>
  </tr>
  <tr>
    <td><a class="job_title_link" href="/apply/jobs/details/BBB222?&">Job B</a></td>
...
def test_parse_api_jobs_table_rows():
    ...
    assert by_url["https://electrogrup.applytojob.com/apply/jobs/details/AAA111"]["location"] == ["Cluj-Napoca"]
    assert by_url["https://electrogrup.applytojob.com/apply/jobs/details/BBB222"]["location"] == ["Bucuresti"]
...
def test_map_to_job_model_adds_company_and_status():
    raw = {"url": "https://electrogrup.applytojob.com/apply/jobs/details/ABC123",
           "title": "SCADA Engineer", "location": ["Cluj-Napoca"]}
    index.COMPANY_NAME = "ELECTROGRUP S.A."
    job = index.map_to_job_model(raw, "9256208")
    assert job["company"] == "ELECTROGRUP S.A."
    assert job["cif"] == "9256208"
    assert job["status"] == "scraped"
    assert job["location"] == ["Cluj-Napoca"]


def test_transform_jobs_for_solr_keeps_required_fields():
    jobs = [{"url": "https://x/job", "title": "Test Job", "location": ["Cluj-Napoca"],
             "company": "ELECTROGRUP S.A.", "cif": "9256208"}]
    transformed = index.transform_jobs_for_solr({"company": "ELECTROGRUP S.A.", "jobs": jobs})
    assert len(transformed["jobs"]) == 1
    t = transformed["jobs"][0]
    assert t["url"]
    assert t["title"]
    assert t["location"] == ["Cluj-Napoca"]
    assert t["company"] == "ELECTROGRUP S.A."
...
def test_generate_jobs_markdown(tmp_path, company_config):
    jobs = [{"url": "https://x/job", "title": "Inginer Ofertare Energetic",
             "company": "ELECTROGRUP S.A.", "cif": "9256208",
             "location": ["Cluj-Napoca"], "workmode": "on-site"}]
...
def test_main_dry_run_writes_summary(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(index, "validate_and_get_company", lambda: {
        "company": "ELECTROGRUP S.A.", "cif": "9256208", "status": "active",
        "address": "CLUJ-NAPOCA"})
