"""
ELECTROGRUP S.A. applytojob Scraper — derived from the Python template.

Scrapes ELECTROGRUP S.A. job listings from the applytojob board
(electrogrup.applytojob.com) filtered by department, validates the company
via ANAF, and publishes jobs/company data to peviitor.ro through the v1 API
...
def extract_location(location_text):
    """Extracts the city from a location string like 'Cluj-Napoca, Romania'."""
...
def parse_api_jobs(html):
    ...
    for link in soup.find_all("a", class_="job_title_link"):
        ...
        if row:
            ...
            loc_tag = link.find_parent("div", class_="jobs_row")
            if loc_tag:
...
def main(root=None):
    ...
    print(f"Jobs scraped from the ELECTROGRUP SA applytojob board: {scraped_count}")
